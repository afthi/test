...
def custom_dataloader():
    image_dir = 'C:\\Users\\fertig\\Documents\\FILES\\data\\datasets\\nuscenes\\999_image_visualization\\20221202_test_val_set\\error_signals_diff_x_y_no_padding_cut_xaxis\\'
    ...
    image_width  = 256  # 256
    XData = np.zeros((max_idx+1, 256, 256, 3), dtype=np.uint8)
    for i in range(max_idx+1):
        image = np.array(io.imread(image_dir+str(i)+'.png')
                         ).astype(np.uint8)
        XData[i] = image[:,:,0:3]
...
def main(argv):
    ...
    image_height = 256   # needs to be 2^n
    image_width = 256    # needs to be 2^n
    # The Images will be rescaled to the given height and width
    data_dir = './dataVisualisation'
